...
class Layer(object):
    ...
    def backward(self, d_outputs):
        ...
        #     N is the number of data points in this batch.
        #     H is the number of hidden units in this layer.
        d_scores = self.d_activate(self.a)

        # self.d_b:
        #     Gradient of the loss w.r.t the bias
        #     A matrix of shape (1, H)
        #     H is the number of hidden units in this layer.
        self.d_b = np.sum(np.multiply(d_outputs, d_scores), axis = 0, keepdims = True)
        ...
        #     H_-1 is the number of hidden units in previous layer
        #     H is the number of hidden units in this layer.        
        self.d_w = np.dot(np.transpose(self.inputs), np.multiply(d_outputs, d_scores))
        ...
        d_inputs = np.dot(np.multiply(d_outputs, d_scores), np.transpose(self.w))
